...
import { customFoodToSearchResult } from '@/lib/food-search';
...
export async function GET() {
  try {
    ...
    return NextResponse.json({ customFoods: customFoods.map(customFoodToSearchResult) });
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ customFood: customFoodToSearchResult(customFood) });
  } catch (error) {
