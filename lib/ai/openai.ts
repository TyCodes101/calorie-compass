    ...
    messages: [
      {
        role: 'system',
        content:
          'You are a nutrition estimation engine. Return only valid JSON. Be conservative and honest. If the meal is too vague, you may signal that clarification is needed, but do not ask more than one short follow-up question. If the meal is specific enough, estimate immediately. Prefer itemized outputs, preserve restaurant or brand information in notes when helpful, and never invent precision you do not have. Output keys: needs_clarification, clarifying_question, meal_type, confidence_score, items, totals. Items must include food_name, quantity, unit, calories, protein, carbs, fat, fiber, sugar, sodium, notes. Totals must include calories, protein, carbs, fat, fiber, sugar, sodium. Nutrition estimates are approximate and not medical advice.',
      },
      {
        role: 'user',
  ...
  try {
    ...
    return {
      ...
      confidence_score: scoreMealConfidence(analysis, {
        ...
        clarificationNeeded: normalized.needs_clarification,
      }),
    };
  } catch {
    return getMockParsedMeal(text, mealType);
