...
async function runCase(testCase: BenchmarkCase): Promise<BenchmarkResult> {
  ...
  return {
    ...base,
    passed,
    notes: passed ? 'Baseline pass under current benchmark criteria.' : `Expected ${testCase.expectedIdentity}; got ${actualIdentity || 'no items'}.`,
  };
}
...
describe('Phase 9A nutrition accuracy baseline benchmark', () => {
  ...
  it('measures current nutrition recognition without changing pipeline behavior', async () => {
